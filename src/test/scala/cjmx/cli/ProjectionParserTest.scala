...
import cjmx.util.jmx.{Beans => B, _}
...
class ProjectionParserTest extends FunSuite with Matchers {
  ...
  private def parse(str: String): Either[String, Seq[Attribute] => Seq[Attribute]] =
    Parser.parse(str, JMXParsers.Projection(ManagementFactory.getPlatformMBeanServer, Some(B.Query.byName("java.lang:type=Memory"))))
